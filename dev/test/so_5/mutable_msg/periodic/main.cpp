/*
 * A test for sending delayed mutable message to mpmc mbox.
 */
		...
		try
		{
			m_timer = so_5::send_periodic< so_5::mutable_msg<first> >(
					*this,
					...
					"an exception must be thrown before this point (sobj)" );
		}
		catch( const so_5::exception_t & x )
		{
			ensure( so_5::rc_mutable_msg_cannot_be_periodic ==
					x.error_code(),
					"an rc_mutable_msg_cannot_be_periodic expected" );
		}
		...
		try
		{
			m_timer = so_5::send_periodic< so_5::mutable_msg<first> >(
					*this,
					...
					"an exception must be thrown before this point (user)" );
		}
		catch( const so_5::exception_t & x )
		{
			ensure( so_5::rc_mutable_msg_cannot_be_periodic ==
