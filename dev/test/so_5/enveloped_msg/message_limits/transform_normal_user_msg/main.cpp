...
struct msg_one final {
	std::string m_one;
	std::string m_two;
	std::string m_three;
};

struct msg_two final {
	std::string m_value;
};
...
public:
...

	{
		so_default_state().event( m_mbox,
				[this]( mhood_t<msg_one> cmd ) {
					m_trace.get().append( "msg_one{" + cmd->m_one + "."
							+ cmd->m_two + "." + cmd->m_three + "};" );
				} )
			.event(
	...
	{
		so_5::send<msg_one>( m_mbox, "One", "Two", "Three" );
		so_5::send<msg_one>( m_mbox, "Four", "Five", "Six" );
	}
};
...
run_test()
{
	...
	ensure_or_die(
			"mbox[2]:transform;"
			"mbox[1]:pre_invoke;msg_one{One.Two.Three};mbox[1]:post_invoke;"
			"msg_two{[Six.Five.Four]};"
					== trace.content(),
