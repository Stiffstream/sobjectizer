...
class a_consumer_t final : public so_5::agent_t
	...
	private:
		bool m_msg_data_received{ false };

		void
		evt_data( mhood_t<msg_data> cmd )
			{
				std::cout << "\n\n\n\n" << std::flush << "data: " << cmd->m_v << std::endl;
				m_msg_data_received = true;
			}

		void
		evt_signal( mhood_t<msg_signal> )
			{
				ensure_or_die( !m_msg_data_received,
						"msg_data should not be received before msg_signal" );
