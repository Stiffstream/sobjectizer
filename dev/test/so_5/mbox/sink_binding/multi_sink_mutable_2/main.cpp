/*
 * A check that only one bind can be made for a mutable message.
 */
...
introduce_test_coop( so_5::environment_t & env )
	{
		env.introduce_coop( []( so_5::coop_t & coop ) {
				...
				auto * consumer_1 = coop.make_agent< a_consumer_t >();
				auto consumer_1_msink = so_5::wrap_to_msink( consumer_1->so_direct_mbox() );

				auto * consumer_2 = coop.make_agent< a_consumer_t >();
				auto consumer_2_msink = so_5::wrap_to_msink( consumer_2->so_direct_mbox() );

				auto * binding = coop.take_under_control(
						std::make_unique< so_5::multi_sink_binding_t<> >() );
				
				binding->bind< so_5::mutable_msg<msg_data> >( data_mbox, consumer_1_msink );

				bool error_happens = false;
				try
					{
						binding->bind< so_5::mutable_msg<msg_data> >(
								data_mbox, consumer_2_msink );
					}
				catch( const so_5::exception_t & x )
					{
						error_happens = ( so_5::rc_evt_handler_already_provided
								== x.error_code() );
						if( error_happens )
				...
				ensure_or_die( error_happens,
						"the second bind should throw an exception" );

				binding->bind< msg_signal >( data_mbox, consumer_1_msink );
			} );
	}
