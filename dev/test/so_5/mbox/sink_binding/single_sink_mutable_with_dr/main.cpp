/*
 * A simple case for single_sink_binding_t and mutable msg and delivery filter.
 */
		...
		so_evt_start() override
			{
				...
				so_5::send< so_5::mutable_msg<msg_data> >( m_dest, 0 );
				so_5::send< so_5::mutable_msg<msg_data> >( m_dest, 1 );
				so_5::send< so_5::mutable_msg<msg_data> >( m_dest, 10 );
				so_5::send< so_5::mutable_msg<msg_data> >( m_dest, 11 );
		...
		evt_data( mutable_mhood_t<msg_data> cmd )
			{
				...
				ensure_or_die( 0 <= cmd->m_v && cmd->m_v <= 10,
						"value in msg_data is out of range: " + std::to_string( cmd->m_v ) );
				m_messages_received += 1u;
			}

		void
		evt_signal( mhood_t<msg_signal> )
			{
				ensure_or_die( 3 == m_messages_received,
...
introduce_test_coop( so_5::environment_t & env )
	{
		env.introduce_coop( []( so_5::coop_t & coop ) {
				...
				msg_data_binding->bind< so_5::mutable_msg<msg_data> >(
						data_mbox,
						so_5::wrap_to_msink( consumer->so_direct_mbox() ),
						[]( const msg_data & msg ) {
							return 0 <= msg.m_v && msg.m_v <= 10;
						} );
