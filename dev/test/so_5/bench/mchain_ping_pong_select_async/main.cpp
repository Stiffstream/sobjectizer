...
pinger_thread_func(
	...
	{
		so_5::cpp_coro::this_thread_scheduler_t scheduler{ ping_ch->environment() };
		...
		for( unsigned int pong_received = 0; pong_received < request_count;
				++pong_received )
			{
				...
				scheduler.sync_wait( so_5::cpp_coro::select(
						scheduler,
		...
		ensure_or_die( calls == request_count,
				"mismatch for pinger: calls=" + std::to_string( calls )
...
main( int argc, char ** argv )
{
	try
	{
		...
		duration_meter_t meter{ "ping-pong on async select" };
