...
struct msg_take_tid final : public so_5::message_t
	{
		...
		explicit msg_take_tid(
			so_5::current_thread_id_t tid,
			who_t who )
			:	m_tid{ tid }
			,	m_who{ who }
			{}
...
class a_collector_t final : public so_5::agent_t
	{
		so_5::current_thread_id_t m_self_tid;
	...
	public:
		...
		so_evt_start() override
			{
				m_self_tid = so_5::query_current_thread_id();
			}
		...
		evt_take_tid( mhood_t< msg_take_tid > cmd )
			{
				switch( cmd->m_who )
					{
					case who_t::parent:
						ensure_or_die( m_self_tid != cmd->m_tid,
								"parent has to work on a separate thread!" );
					break;

					case who_t::child:
						ensure_or_die( m_self_tid == cmd->m_tid,
								"child has to work on the same thread!" );
					break;
					}
			}
	};
		...
		so_evt_start() override
			{
				so_5::send< msg_take_tid >( m_collector_mbox,
						so_5::query_current_thread_id(), who_t::child );
...
class a_parent_t final : public so_5::agent_t
	...
	public:
		...
		so_evt_start() override
			{
				so_5::send< msg_take_tid >( m_collector_mbox,
						so_5::query_current_thread_id(), who_t::parent );
