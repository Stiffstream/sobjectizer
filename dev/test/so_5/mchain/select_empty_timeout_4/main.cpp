...
do_test()
{
	...
	for( const auto & p : params )
	{
		...
		auto ch4 = env.environment().create_mchain( p.second );

		bool hello_received = false;
		bool hello_again_received = false;
		bool outgoing_sent = false;
		...
		auto r = so_5::select(
				...
				send_case( ch4,
					so_5::message_holder_t< outgoing >::make(),
					[&outgoing_sent]() {
						outgoing_sent = true;
					} ),
