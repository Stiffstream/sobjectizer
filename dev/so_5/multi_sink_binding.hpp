...
//FIXME: document this!
enum class key_category_t : char
	{
		lower_bound = 0,
		normal,
		upper_bound
...
struct binding_key_t
	{
		mbox_id_t m_mbox_id;
		msink_t m_msink;
		std::type_index m_msg_type;
		key_category_t m_key_category;

		binding_key_t(
			//NOTE: category can't be key_category_t::normal!
			key_category_t key_category,
			mbox_id_t mbox_id,
			msink_t msink )
			:	m_mbox_id{ mbox_id }
			,	m_msink{ std::move(msink) }
			,	m_msg_type{ typeid(void) }
			,	m_key_category{ key_category }
			{}
		...
		binding_key_t(
			...
			,	m_msink{ std::move(msink) }
			,	m_msg_type{ msg_type }
			,	m_key_category{ key_category_t::normal }
			{}
	};

[[nodiscard]]
bool
operator<( const binding_key_t & a, const binding_key_t & b )
	{
		const auto tie = [](
			const binding_key_t & src,
			const so_5::impl::msink_const_ref_for_comparison_t & msink_ref ) {
				return std::tie(
						src.m_mbox_id,
						msink_ref,
						src.m_key_category,
		...
		const so_5::impl::msink_const_ref_for_comparison_t a_msink{ a.m_msink };
		const so_5::impl::msink_const_ref_for_comparison_t b_msink{ b.m_msink };

		return tie( a, a_msink ) < tie( b, b_msink );
	}

//FIXME: document this!
using bindings_map_t = std::map< binding_key_t, single_sink_binding_t >;
...
class actual_binding_handler_t
	{
		...
		do_actual_bind(
			...
			Single_Sink_Modificator && single_sink_modificator )
			{
				insertion_it_with_auto_erase_if_not_committed_t it{ m_bindings,
						binding_key_t{ from->id(), dest, msg_type }
					};
				...
				single_sink_modificator( msg_type, it->second );
...
//FIXME: methods do_bind have too many similar lines, can they be refactored?
			...
			const msink_t & dest ) noexcept
		{
			...
			m_bindings.erase( binding_key_t{ from->id(), dest, msg_type } );
		}
		...
		{
			m_bindings.erase(
					m_bindings.lower_bound( binding_key_t{
							key_category_t::lower_bound,
							from->id(),
							dest
						} ),
					m_bindings.upper_bound( binding_key_t{
							key_category_t::upper_bound,
							from->id(),
							dest
						} )
				);
...
using multi_sink_v2::multi_sink_binding_t;
